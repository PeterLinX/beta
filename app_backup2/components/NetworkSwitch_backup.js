...
const getEthBalance = async (net , eth_address) => {
    let base;
    if (net === "MainNet") {
        base = "http://api.blockcypher.com/v1/eth/main/addrs/";
    } else {
        base = "http://api.blockcypher.com/v1/eth/test3/addrs/";
    }
...
const initiateEthGetBalance = async (dispatch, net, eth_address) => {
    let base;
    syncLtcTransactionHistory(dispatch,net,eth_address);
    const eth_balance = getEthBalance(net,eth_address);
    setLtcBalance(eth_balance);
