...
const apiURL = val => {
  return "https://min-api.cryptocompare.com/data/price?fsym=TKY&tsyms=USD";
};
...
const validateForm = (dispatch, tky_balance) => {
  ...
  if (
    ...
    dispatch(sendEvent(false, "You cannot send fractional amounts of TKY."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  } else if (parseInt(sendAmount.value) > tky_balance) {
    // check for value greater than account balance
    dispatch(sendEvent(false, "You do not have enough TKY to send."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }  else if (parseFloat(sendAmount.value) <= 0) {
    // check for negative asset
    dispatch(sendEvent(false, "You cannot send negative amounts of  TKY."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }
  return true;
...
// perform send transaction for TKY
const sendTkyTransaction = async (dispatch, net, selfAddress, wif) => {
  ...
  if (validateForm(dispatch,tky_balance) === true) {
      if (tky_balance <= sendAmount.value) {
          dispatch(sendEvent(false, "You are trying to send more TKY than you have available."));
          setTimeout(() => dispatch(clearTransactionEvent()), 2000);
          return true;
      } else {
          dispatch(sendEvent(true, "Sending TKY...\n"));
          try {
              ...
              console.log("sending tky response=" + response.result);
              if (!response.result) {
              ...
              console.log("sending tky =" + err.message);
        ...
        >
          <div>
            ...
            <div className="center modal-alert top-20">
              <strong>Confirm sending {sendAmount} TKY to {sendAddress}</strong>
            </div>
...
class SendTKY extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: true,
      ...
      <div>
        ...
        <div id="send">
            ...
            <div className="top-20">
              <div className="col-xs-9">
                <input
                  ...
                  id="center"
                  placeholder="Enter a valid TKY public address here"
                  ref={node => {
                    sendAddress = node;
                  }}
                />
                  ...
                  <button
                    ...
                    onClick={() => {
                        ...
                        if (parseFloat(sendAmount.value) <= 0) {
                            dispatch(sendEvent(false, "You cannot send negative amounts of an TKY."));
                            setTimeout(() => dispatch(clearTransactionEvent()), 1000);
                            return false;
                        }
          ...
          <div className="send-notice">
            <p>
              Sending TKY requires a balance of 1 GAS+. Only send TKY to a valid
              address that supports NEP5+ tokens on the NEO blockchain. When
              sending TKY to an exchange please ensure the address supports TKY
              tokens.
            </p>
...
SendTKY = connect(mapStateToProps)(SendTKY);

export default SendTKY;
