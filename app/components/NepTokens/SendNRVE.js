...
let sendAddress, sendAmount, confirmButton, scriptHash, nrve_usd, gas_usd;
...
const apiURL = val => {
  return "https://min-api.cryptocompare.com/data/price?fsym=NRVE&tsyms=USD";
};
...
const validateForm = (dispatch, nrv_balance) => {
  ...
  if (
    ...
    dispatch(sendEvent(false, "You cannot send fractional amounts of Nrve."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  } else if (parseInt(sendAmount.value) > nrv_balance) {
    // check for value greater than account balance
    dispatch(sendEvent(false, "You do not have enough NRVE to send."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  } else if (parseFloat(sendAmount.value) <= 0) {
    // check for negative asset
    dispatch(sendEvent(false, "You cannot send negative amounts of NRVE."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }
  return true;
...
// perform send transaction for NRVE
const sendNrveTransaction = async (dispatch, net, selfAddress, wif) => {
  ...
  if (validateForm(dispatch,nrve_balance) === true) {
      if (nrve_balance <= sendAmount.value) {
          dispatch(sendEvent(false, "You are trying to send more NRVE than you have available."));
          setTimeout(() => dispatch(clearTransactionEvent()), 2000);
          return true;
      } else {
          dispatch(sendEvent(true, "Sending NRVE...\n"));
          try {
              ...
              console.log("sending nrve response=" + response.result);
              if (!response.result) {
              ...
              console.log("sending nrve =" + err.message);
        ...
        >
          <div>
            ...
            <div className="center modal-alert top-20">
              <strong>Confirm sending {sendAmount} NRVE to {sendAddress}</strong>
            </div>
...
class SendNRVE extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: true,
      ...
      <div>
        ...
        <div id="send">
          <div className="row dash-chart-panel">
            ...
            ).format("0,0.0000")} <span className="dbc-price"> NRVE</span></span><br />
            ...
            <div className="top-20">
              <div className="col-xs-9">
                <input
                  ...
                  id="center"
                  placeholder="Enter a valid NRVE public address here"
                  ref={node => {
                    sendAddress = node;
                  }}
                />
                  ...
                  <button
                    ...
                    onClick={() => {
                        ...
                        if (parseFloat(sendAmount.value) <= 0) {
                            dispatch(sendEvent(false, "You cannot send negative amounts of an NRVE."));
                            setTimeout(() => dispatch(clearTransactionEvent()), 1000);
                            return false;
                        }
          ...
          <div className="send-notice">
            <p>
              Sending NRVE requires a balance of 1 GAS+. Only send NRVE to a valid address that supports NEP5+ tokens on the NEO blockchain. When sending NRVE to an exchange please ensure the address supports NRVE tokens.
            </p>
...
SendNRVE = connect(mapStateToProps)(SendNRVE);

export default SendNRVE;
