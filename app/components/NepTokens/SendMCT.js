...
let sendAddress, sendAmount, confirmButton, scriptHash, mct_usd, gas_usd;
...
const apiURL = val => {
  return "https://min-api.cryptocompare.com/data/price?fsym=MCT&tsyms=USD";
};
...
const validateForm = (dispatch, mct_balance) => {
  ...
  if (
    parseInt(sendAmount.value) > mct_balance) {
    // check for value greater than account balance
    dispatch(sendEvent(false, "You do not have enough MCT to send."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }  else if (parseFloat(sendAmount.value) <= 0) {
    // check for negative asset
    dispatch(sendEvent(false, "You cannot send negative amounts of MCT."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }
  return true;
...
// perform send transaction for MCT
const sendMctTransaction = async (dispatch, net, selfAddress, wif) => {
  ...
  if (mct_balance <= sendAmount.value) {
    dispatch(sendEvent(false, "You are trying to send more MCT than you have available."));
		setTimeout(() => dispatch(clearTransactionEvent()), 2000);
		return true;
  } else {
    dispatch(sendEvent(true, "Sending MCT...\n"));
    try {
      ...
      console.log("sending mct response=" + response.result);
      if (!response.result) {
      ...
      console.log("sending mct =" + err.message);
        ...
        >
          <div>
            ...
            <div className="center modal-alert top-20">
              <strong>Confirm sending {sendAmount} MCT to {sendAddress}</strong>
            </div>
...
class SendMCT extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: true,
        ...
        <div id="send">
          <div className="row dash-chart-panel">
            <div className="col-xs-9">
              ...
              />
              <h2>Master Contract Tokens</h2>
            </div>

            <div className="col-xs-3 center">

            <span className="font-16">{numeral(
              Math.floor(this.props.mct * 100000) / 100000
            ).format("0,0[.][0000]")} <span className="ltc-price"> MCT</span></span><br />
            ...
            <div className="top-20">
              <div className="col-xs-9">
                <input
                  ...
                  id="center"
                  placeholder="Enter a valid MCT public address here"
                  ref={node => {
                    sendAddress = node;
                  }}
                />
                  ...
                  <button
                    ...
                    onClick={() => {
                        ...
                        if (parseFloat(sendAmount.value) <= 0) {
                            dispatch(sendEvent(false, "You cannot send negative amounts of an MCT."));
                            setTimeout(() => dispatch(clearTransactionEvent()), 1000);
                            return false;
                        }
          ...
          <div className="send-notice">
            <p>
              Sending Master Contract (MCT) NEP5 tokens require a balance of 0.00000001 GAS+. Only send MCT to a valid address that supports NEP5+ tokens on the NEO blockchain. When sending MCT to an exchange please ensure the address supports MCT tokens.
            </p>
            </div>
          </div>
        </div>
...
SendMCT = connect(mapStateToProps)(SendMCT);

export default SendMCT;
