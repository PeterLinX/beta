...
const validateForm = (dispatch, ont_balance) => {
  ...
  if (
    ...
    dispatch(sendEvent(false, "You cannot send fractional amounts of Ont."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  } else if (parseInt(sendAmount.value) > ont_balance) {
    // check for value greater than account balance
    dispatch(sendEvent(false, "You do not have enough ONT to send."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
...
const sendOntTransaction = async (dispatch, net, selfAddress, wif) => {
  ...
  if (validateForm(dispatch,ont_balance) === true) {
        ...
        >
          <div>
            ...
            <div className="center modal-alert top-20">
              <strong>Confirm sending {sendAmount} ONT to {sendAddress}</strong>
            </div>
