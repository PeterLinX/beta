...
const apiURL = val => {
  return "https://min-api.cryptocompare.com/data/price?fsym=GAGA&tsyms=USD";
};
...
const validateForm = (dispatch, gaga_balance) => {
  ...
  if (
    parseInt(sendAmount.value) > gaga_balance) {
    // check for value greater than account balance
    dispatch(sendEvent(false, "You do not have enough GAGA to send."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  } else if (parseFloat(sendAmount.value) <= 0) {
    // check for negative asset
    dispatch(sendEvent(false, "You cannot send negative amounts of GAGA."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }
  return true;
...
// perform send transaction for GAGA
const sendGagaTransaction = async (dispatch, net, selfAddress, wif) => {
  ...
  if (validateForm(dispatch, gaga_balance) === true) {
      if (gaga_balance <= sendAmount.value) {
          dispatch(sendEvent(false, "You are trying to send more GAGA than you have available."));
          setTimeout(() => dispatch(clearTransactionEvent()), 2000);
          return true;
      } else {
          dispatch(sendEvent(true, "Sending GAGA...\n"));
          try {
              ...
              console.log("sending gaga response=" + response.result);
              if (!response.result) {
              ...
              console.log("sending gaga =" + err.message);
        ...
        >
          <div>
            ...
            <div className="center modal-alert top-20">
              <strong>Confirm sending {sendAmount} GAGA to {sendAddress}</strong>
            </div>
...
class SendGAGA extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: true,
            ...
            <div className="top-20">
              <div className="col-xs-9">
                <input
                  ...
                  id="center"
                  placeholder="Enter a valid GAGA public address here"
                  ref={node => {
                    sendAddress = node;
                  }}
                />
                  ...
                  <button
                    ...
                    onClick={() => {
                        ...
                        if (parseFloat(sendAmount.value) <= 0) {
                            dispatch(sendEvent(false, "You cannot send negative amounts of an GAGA."));
                            setTimeout(() => dispatch(clearTransactionEvent()), 1000);
                            return false;
                        }
...
SendGAGA = connect(mapStateToProps)(SendGAGA);

export default SendGAGA;
