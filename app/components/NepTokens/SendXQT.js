...
let sendAddress, sendAmount, confirmButton, scriptHash, xqt_usd, gas_usd;
...
const apiURL = val => {
  return "https://min-api.cryptocompare.com/data/price?fsym=XQT&tsyms=USD";
};
...
const validateForm = (dispatch, xqt_balance) => {
  ...
  if (
    parseInt(sendAmount.value) > xqt_balance) {
    // check for value greater than account balance
    dispatch(sendEvent(false, "You do not have enough XQT to send."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }  else if (parseFloat(sendAmount.value) <= 0) {
    // check for negative asset
    dispatch(sendEvent(false, "You cannot send negative amounts of XQT."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }
  return true;
...
const sendXqtTransaction = async (dispatch, net, selfAddress, wif) => {
  ...
  if (xqt_balance <= sendAmount.value) {
    dispatch(sendEvent(false, "You are trying to send more XQT than you have available."));
		setTimeout(() => dispatch(clearTransactionEvent()), 2000);
		return true;
  } else {
    dispatch(sendEvent(true, "Sending XQT...\n"));
    try {
        ...
        >
          <div>
            ...
            <div className="center modal-alert top-20">
              <strong>Confirm sending {sendAmount} XQT to {sendAddress}</strong>
            </div>
...
class SendXQT extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: true,
  ...
  render() {
    ...
    return (
      <div>
          ...
          <li className="active">Quarteria</li>
          </ol>
          </div>
        ...
        <div id="send">
          <div className="row dash-chart-panel">
            ...
            ).format("0,0[.][0000]")} <span id="no-inverse" className="ltc-price"> XQT</span></span><br />
            ...
            <div className="top-20">
              <div className="col-xs-9">
                <input
                  ...
                  id="center"
                  placeholder="Enter a valid XQT public address here"
                  ref={node => {
                    sendAddress = node;
                  }}
                />
                  ...
                  <button
                    ...
                    onClick={() => {
                        ...
                        if (parseFloat(sendAmount.value) <= 0) {
                            dispatch(sendEvent(false, "You cannot send negative amounts of an XQT."));
                            setTimeout(() => dispatch(clearTransactionEvent()), 1000);
                            return false;
                        }
          ...
          <div className="send-notice">
            <p>
              Sending Quarteria (XQT) NEP5 tokens require a balance of 0.00000001 GAS+. Only send XQT to a valid address that supports NEP5+ tokens on the NEO blockchain. When sending XQT to an exchange please ensure the address supports XQT tokens.
            </p>
            </div>
          </div>
        </div>
...
SendXQT = connect(mapStateToProps)(SendXQT);
export default SendXQT;
