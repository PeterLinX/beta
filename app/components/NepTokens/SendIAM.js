...
const apiURL = val => {
  return "https://min-api.cryptocompare.com/data/price?fsym=IAM&tsyms=USD";
};
...
const validateForm = (dispatch, iam_balance) => {
  ...
  if (
    parseInt(sendAmount.value) > iam_balance) {
    // check for value greater than account balance
    dispatch(sendEvent(false, "You do not have enough IAM to send."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  } else if (parseFloat(sendAmount.value) <= 0) {
    // check for negative asset
    dispatch(sendEvent(false, "You cannot send negative amounts of IAM."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }
  return true;
...
// perform send transaction for IAM
const sendIamTransaction = async (dispatch, net, selfAddress, wif) => {
  ...
  if (validateForm(dispatch, iam_balance) === true) {
      if (iam_balance <= sendAmount.value) {
          dispatch(sendEvent(false, "You are trying to send more IAM than you have available."));
          setTimeout(() => dispatch(clearTransactionEvent()), 2000);
          return true;
      } else {
          dispatch(sendEvent(true, "Sending IAM...\n"));
          try {
              ...
              console.log("sending iam response=" + response.result);
              if (!response.result) {
              ...
              console.log("sending iam =" + err.message);
        ...
        >
          <div>
            ...
            <div className="center modal-alert top-20">
              <strong>Confirm sending {sendAmount} IAM to {sendAddress}</strong>
            </div>
...
class SendIAM extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: true,
  ...
  render() {
    ...
    return (
      <div>
          ...
          <li className="active">Bridge Protocol</li>
          </ol>
          </div>
        ...
        <div id="send">
          <div className="row dash-chart-panel">
            ...
            ).format("0,0[.][0000]")} <span id="no-inverse" className="qlink-price"> IAM</span></span><br />
            ...
            <div className="top-20">
              <div className="col-xs-9">
                <input
                  ...
                  id="center"
                  placeholder="Enter a valid IAM public address here"
                  ref={node => {
                    sendAddress = node;
                  }}
                />
                  ...
                  <button
                    ...
                    onClick={() => {
                        ...
                        if (parseFloat(sendAmount.value) <= 0) {
                            dispatch(sendEvent(false, "You cannot send negative amounts of an IAM."));
                            setTimeout(() => dispatch(clearTransactionEvent()), 1000);
                            return false;
                        }
          ...
          <div className="send-notice">
            <p>
              Sending Bridge Protocol (IAM) NEP5 tokens require a balance of 0.00000001 GAS+. Only send IAM to a valid address that supports NEP5+ tokens on the NEO blockchain. When sending IAM to an exchange please ensure the address supports IAM tokens.
            </p>
            </div>
          </div>
        </div>
...
SendIAM = connect(mapStateToProps)(SendIAM);

export default SendIAM;
