...
let sendAddress, sendAmount, confirmButton, scriptHash, aph_usd, gas_usd;
...
const apiURL = val => {
  return "https://min-api.cryptocompare.com/data/price?fsym=aph&tsyms=USD";
};
...
const validateForm = (dispatch, aph_balance) => {
  ...
  if (
    parseInt(sendAmount.value) > aph_balance) {
    // check for value greater than account balance
    dispatch(sendEvent(false, "You do not have enough APH to send."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  } else if (parseFloat(sendAmount.value) <= 0) {
    // check for negative asset
    dispatch(sendEvent(false, "You cannot send negative amounts of APH."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }
  return true;
...
// perform send transaction for APH
const sendAphTransaction = async (dispatch, net, selfAddress, wif) => {
  ...
  if (validateForm(dispatch, aph_balance) === true) {
      if (aph_balance <= sendAmount.value) {
          dispatch(sendEvent(false, "You are trying to send more APH than you have available."));
          setTimeout(() => dispatch(clearTransactionEvent()), 2000);
          return true;
      } else {
          dispatch(sendEvent(true, "Sending APH...\n"));
          try {
              ...
              console.log("sending aph response=" + response.result);
              if (!response.result) {
              ...
              console.log("sending aph =" + err.message);
        ...
        >
          <div>
            ...
            <div className="center modal-alert top-20">
              <strong>Confirm sending {sendAmount} APH to {sendAddress}</strong>
            </div>
...
class SendAPH extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: true,
  ...
  render() {
    ...
    return (
      <div>
          ...
          <li className="active">Aphelion</li>
          </ol>
          </div>
        ...
        <div id="send">
          <div className="row dash-chart-panel">
            ...
            ).format("0,0[.][0000]")} <span id="no-inverse" className="qlink-price"> APH</span></span><br />
            <span className="market-price">{numeral(this.props.aph * this.props.marketAPHPrice).format("$0,0.00")} USD</span>
            </div>
            ...
            <div className="top-20">
              <div className="col-xs-9">
                <input
                  ...
                  id="center"
                  placeholder="Enter a valid APH public address here"
                  ref={node => {
                    sendAddress = node;
                  }}
                />
                  ...
                  <button
                    ...
                    onClick={() => {
                        ...
                        if (parseFloat(sendAmount.value) <= 0) {
                            dispatch(sendEvent(false, "You cannot send negative amounts of an APH."));
                            setTimeout(() => dispatch(clearTransactionEvent()), 1000);
                            return false;
                        }
          ...
          <div className="send-notice">
            <p>
              Sending Aphelion (APH) NEP5 tokens require a balance of 0.00000001 GAS+. Only send APH to a valid address that supports NEP5+ tokens on the NEO blockchain. When sending APH to an exchange please ensure the address supports APH tokens.
            </p>
            </div>
          </div>
        </div>
...
SendAPH = connect(mapStateToProps)(SendAPH);

export default SendAPH;
