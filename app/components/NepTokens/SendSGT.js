...
let sendAddress, sendAmount, confirmButton, scriptHash, sgt_usd, gas_usd;
...
const apiURL = val => {
  return "https://min-api.cryptocompare.com/data/price?fsym=SGT&tsyms=USD";
};
...
const validateForm = (dispatch, sgt_balance) => {
  ...
  if (
      parseInt(sendAmount.value) > sgt_balance) {
    // check for value greater than account balance
    dispatch(sendEvent(false, "You do not have enough SGT to send."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
...
// perform send transaction for SGT
const sendSgtTransaction = async (dispatch, net, selfAddress, wif) => {
  ...
  if (validateForm(dispatch,sgt_balance) === true) {
      if (sgt_balance <= sendAmount.value) {
          dispatch(sendEvent(false, "You are trying to send more SGT than you have available."));
          setTimeout(() => dispatch(clearTransactionEvent()), 2000);
          return true;
      } else {
          dispatch(sendEvent(true, "Sending SGT...\n"));
          try {
              ...
              console.log("sending sgt response=" + response.result);
              if (!response.result) {
              ...
              console.log("sending SGT =" + err.message);
        ...
        >
          <div>
            ...
            <div className="center modal-alert top-20">
              <strong>Confirm sending {sendAmount} SGT to {sendAddress}</strong>
            </div>
...
class SendSGT extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: true,
        ...
        <div id="send">
          <div className="row dash-chart-panel">
            <div className="col-xs-9">
              ...
              />
              <h2>SafeGuard Tokens</h2>
            </div>

            <div className="col-xs-3 center">
            <span className="font-16">{numeral(
              Math.floor(this.props.sgt * 100000) / 100000
            ).format("0,0[.][0000]")} <span id="no-inverse" className="rpx-price"> SGT</span></span>
            <br />
            <span className="market-price"> {numeral(this.props.marketSGTPrice).format("$0,0.00")}</span>
            ...
            <div className="top-20">
              <div className="col-xs-9">
                <input
                  ...
                  id="center"
                  placeholder="Enter a valid SGT public address here"
                  ref={node => {
                    sendAddress = node;
                  }}
                />
                  ...
                  <button
                    ...
                    onClick={() => {
                        ...
                        if (parseFloat(sendAmount.value) <= 0) {
                            dispatch(sendEvent(false, "You cannot send negative amounts of an SGT."));
                            setTimeout(() => dispatch(clearTransactionEvent()), 1000);
                            return false;
                        }
          ...
          <div className="send-notice">
            <p>
              Sending SafeGuard (SGT) NEP5 tokens require a balance of 0.00000001 GAS+. Only send SGT to a valid address that supports NEP5+ tokens on the NEO blockchain. When sending SGT to an exchange please ensure the address supports SGT tokens.
            </p>
            </div>
          </div>
        </div>
...
SendSGT = connect(mapStateToProps)(SendSGT);

export default SendSGT;
