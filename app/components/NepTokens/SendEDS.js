...
const apiURL = val => {
  return "https://min-api.cryptocompare.com/data/price?fsym=EDS&tsyms=USD";
};
...
const validateForm = (dispatch, eds_balance) => {
  ...
  if (
    parseInt(sendAmount.value) > eds_balance) {
    // check for value greater than account balance
    dispatch(sendEvent(false, "You do not have enough EDS to send."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }  else if (parseFloat(sendAmount.value) <= 0) {
    // check for negative asset
    dispatch(sendEvent(false, "You cannot send negative amounts of EDS."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }
  return true;
...
// perform send transaction for Loopring NEP EDS
const sendEdsTransaction = async (dispatch, net, selfAddress, wif) => {
  ...
  if (eds_balance <= sendAmount.value) {
    dispatch(sendEvent(false, "You are trying to send more EDS than you have available."));
		setTimeout(() => dispatch(clearTransactionEvent()), 2000);
		return true;
  } else {
    dispatch(sendEvent(true, "Sending EDS...\n"));
    try {
      ...
      console.log("sending eds response=" + response.result);
      if (!response.result) {
      ...
      console.log("sending eds =" + err.message);
        ...
        >
          <div>
            ...
            <div className="center modal-alert top-20">
              <strong>Confirm sending {sendAmount} EDS to {sendAddress}</strong>
            </div>
...
class SendEDS extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: true,
            ...
            <div className="top-20">
              <div className="col-xs-9">
                <input
                  ...
                  id="center"
                  placeholder="Enter a valid EDS public address here"
                  ref={node => {
                    sendAddress = node;
                  }}
                />
                  ...
                  <button
                    ...
                    onClick={() => {
                        ...
                        if (parseFloat(sendAmount.value) <= 0) {
                            dispatch(sendEvent(false, "You cannot send negative amounts of an EDS."));
                            setTimeout(() => dispatch(clearTransactionEvent()), 1000);
                            return false;
                        }
...
SendEDS = connect(mapStateToProps)(SendEDS);
