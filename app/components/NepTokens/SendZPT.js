...
import ZPTChart from "./../NepCharts/ZPTChart";
import NEPQRModalButton from "./../Assets/NEPQRModalButton.js";
import Search from "./../Search";
let sendAddress, sendAmount, confirmButton, scriptHash, zpt_usd, gas_usd;
...
const apiURL = val => {
  return "https://min-api.cryptocompare.com/data/price?fsym=ZPT&tsyms=USD";
};
...
const validateForm = (dispatch, zpt_balance) => {
  ...
  if (
    parseInt(sendAmount.value) > zpt_balance) {
    // check for value greater than account balance
    dispatch(sendEvent(false, "You do not have enough ZPT to send."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  } else if (parseFloat(sendAmount.value) <= 0) {
    // check for negative asset
    dispatch(sendEvent(false, "You cannot send negative amounts of ZPT."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }
  return true;
...
// perform send transaction for ZPT
const sendZptTransaction = async (dispatch, net, selfAddress, wif) => {
  ...
  if (validateForm(dispatch, zpt_balance) === true) {
      if (zpt_balance <= sendAmount.value) {
          dispatch(sendEvent(false, "You are trying to send more ZPT than you have available."));
          setTimeout(() => dispatch(clearTransactionEvent()), 2000);
          return true;
      } else {
          dispatch(sendEvent(true, "Sending ZPT...\n"));
          try {
              ...
              console.log("sending zpt response=" + response.result);
              if (!response.result) {
              ...
              console.log("sending zpt =" + err.message);
        ...
        >
          <div>
            ...
            <div className="center modal-alert top-20">
              <strong>Confirm sending {sendAmount} ZPT to {sendAddress}</strong>
            </div>
...
class SendZPT extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: true,
        ...
        <div id="send">
          <div className="row dash-panel">
            <div className="col-xs-5">
              ...
              <span className="market-price"> {numeral(this.props.marketZPTPrice).format("$0,0.00")} each</span><br />
              <span className="font24">{numeral(
                Math.floor(this.props.zpt * 100000) / 100000
              ).format("0,0[.][0000]")} <span id="no-inverse" className="neo-price"> ZPT</span></span><br />
            ...
            <div className="top-20">
              <div className="col-xs-9">
                <input
                  ...
                  id="center"
                  placeholder="Enter a valid ZPT public address here"
                  ref={node => {
                    sendAddress = node;
                  }}
                />
                  ...
                  <button
                    ...
                    onClick={() => {
                        ...
                        if (parseFloat(sendAmount.value) <= 0) {
                            dispatch(sendEvent(false, "You cannot send negative amounts of an ZPT."));
                            setTimeout(() => dispatch(clearTransactionEvent()), 1000);
                            return false;
                        }
          ...
          <div className="send-notice">
            <p>
              Sending Zeepin (ZPT) NEP5 tokens require a balance of 0.00000001 GAS+. Only send ZPT to a valid address that supports NEP5+ tokens on the NEO blockchain. When sending ZPT to an exchange please ensure the address supports ZPT tokens.
            </p>
            </div>
          </div>
        </div>
...
SendZPT = connect(mapStateToProps)(SendZPT);

export default SendZPT;
