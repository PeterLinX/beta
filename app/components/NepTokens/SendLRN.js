...
let sendAddress, sendAmount, confirmButton, scriptHash, lrn_usd, gas_usd;
...
const apiURL = val => {
  return "https://min-api.cryptocompare.com/data/price?fsym=LRN&tsyms=USD";
};
...
const validateForm = (dispatch, lrn_balance) => {
  ...
  if (
    parseInt(sendAmount.value) > lrn_balance) {
    // check for value greater than account balance
    dispatch(sendEvent(false, "You do not have enough LRN to send."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }  else if (parseFloat(sendAmount.value) <= 0) {
    // check for negative asset
    dispatch(sendEvent(false, "You cannot send negative amounts of LRN."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }
  return true;
...
// perform send transaction for Loopring NEP LRN
const sendLrnTransaction = async (dispatch, net, selfAddress, wif) => {
  ...
  if (lrn_balance <= sendAmount.value) {
    dispatch(sendEvent(false, "You are trying to send more LRN than you have available."));
		setTimeout(() => dispatch(clearTransactionEvent()), 2000);
		return true;
  } else {
    dispatch(sendEvent(true, "Sending LRN...\n"));
    try {
      ...
      console.log("sending lrn response=" + response.result);
      if (!response.result) {
      ...
      console.log("sending lrn =" + err.message);
        ...
        >
          <div>
            ...
            <div className="center modal-alert top-20">
              <strong>Confirm sending {sendAmount} LRN to {sendAddress}</strong>
            </div>
...
class SendLRN extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: true,
  ...
  render() {
    ...
    return (
      <div>
          ...
          <li className="active">Loopring NEP5</li>
          </ol>
          </div>
        ...
        <div id="send">
          <div className="row dash-chart-panel">
            ...
            ).format("0,0[.][0000]")} <span className="ltc-price"> LRN</span></span><br />
            <span className="market-price">{numeral(this.props.lrn * this.props.marketONTPrice).format("$0,0.00")} USD</span>
            </div>
            ...
            <div className="top-20">
              <div className="col-xs-9">
                <input
                  ...
                  id="center"
                  placeholder="Enter a valid LRN public address here"
                  ref={node => {
                    sendAddress = node;
                  }}
                />
                  ...
                  <button
                    ...
                    onClick={() => {
                        ...
                        if (parseFloat(sendAmount.value) <= 0) {
                            dispatch(sendEvent(false, "You cannot send negative amounts of an LRN."));
                            setTimeout(() => dispatch(clearTransactionEvent()), 1000);
                            return false;
                        }
          ...
          <div className="send-notice">
            <p>
              <strong>Notice:</strong> Only send LRN to a valid address that supports NEP5 tokens. Sending Loopring (LRN) requires a balance of 0.00000001 GAS+.  When sending LRN to an exchange please ensure the address supports LRN tokens.
            </p>
            </div>
          </div>
        </div>
...
SendLRN = connect(mapStateToProps)(SendLRN);

export default SendLRN;
