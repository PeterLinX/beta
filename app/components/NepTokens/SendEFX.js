...
const apiURL = val => {
  return "https://min-api.cryptocompare.com/data/price?fsym=EFX&tsyms=USD";
};
...
const validateForm = (dispatch, efx_balance) => {
  ...
  if (
    ...
    dispatch(sendEvent(false, "You cannot send fractional amounts of EFX."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  } else if (asset === "Neo" && parseInt(sendAmount.value) > efx_balance) {
    // check for value greater than account balance
    dispatch(sendEvent(false, "You do not have enough EFX to send."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }  else if (parseFloat(sendAmount.value) <= 0) {
    // check for negative asset
    dispatch(sendEvent(false, "You cannot send negative amounts of EFX."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }
  return true;
...
// perform send transaction for EFX
const sendEfxTransaction = async (dispatch, net, selfAddress, wif) => {
  ...
  if (efx_balance <= sendAmount.value) {
    dispatch(sendEvent(false, "You are trying to send more EFX than you have available."));
		setTimeout(() => dispatch(clearTransactionEvent()), 2000);
		return true;
  } else {
    dispatch(sendEvent(true, "Sending EFX...\n"));
    try {
      ...
      console.log("sending efx response=" + response.result);
      if (!response.result) {
      ...
      console.log("sending efx =" + err.message);
        ...
        >
          <div>
            ...
            <div className="center modal-alert top-20">
              <strong>Confirm sending {sendAmount} EFX to {sendAddress}</strong>
            </div>
...
class SendEFX extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: true,
      ...
      <div>
        ...
        <div id="send">
            ...
            <div className="top-20">
              <div className="col-xs-9">
                <input
                  ...
                  id="center"
                  placeholder="Enter a valid EFX public address here"
                  ref={node => {
                    sendAddress = node;
                  }}
                />
                  ...
                  <button
                    ...
                    onClick={() => {
                        ...
                        if (parseFloat(sendAmount.value) <= 0) {
                            dispatch(sendEvent(false, "You cannot send negative amounts of an EFX."));
                            setTimeout(() => dispatch(clearTransactionEvent()), 1000);
                            return false;
                        }
          ...
          <div className="send-notice">
            <p>
              Sending EFX requires a balance of 1 GAS+. Only send EFX to a valid address that supports NEP5+ tokens on the NEO blockchain. When sending EFX to an exchange please ensure the address supports EFX tokens.
            </p>
...
SendEFX = connect(mapStateToProps)(SendEFX);

export default SendEFX;
