...
const validateForm = (dispatch, swh_balance) => {
  ...
  if (
    ...
    dispatch(sendEvent(false, "You cannot send negative amounts of SWH."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  }
  return true;
...
// perform send transaction for SWH
const sendSwhTransaction = async (dispatch, net, selfAddress, wif) => {
  ...
  if (validateForm(dispatch, swh_balance) === true) {
      if (swh_balance <= sendAmount.value) {
          ...
          dispatch(sendEvent(true, "Sending SWH...\n"));
          try {
              ...
              console.log("sending swh response=" + response.result);
              if (!response.result) {
              ...
              console.log("sending swh =" + err.message);
        ...
        >
          <div>
            ...
            <div className="center modal-alert top-20">
              <strong>Confirm sending {sendAmount} SWH to {sendAddress}</strong>
            </div>
...
class SendSWH extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: true,
  ...
  render() {
    ...
    return (
      <div>
          ...
          <li className="active">Switcheo</li>
          </ol>
          </div>
        ...
        <div id="send">
          <div className="row dash-chart-panel">
            ...
            ).format("0,0[.][0000]")} <span id="no-inverse" className="neo-price"> SWH</span></span><br />
                  ...
                  <button
                    ...
                    onClick={() => {
                        ...
                        if (parseFloat(sendAmount.value) <= 0) {
                            dispatch(sendEvent(false, "You cannot send negative amounts of an SWH."));
                            setTimeout(() => dispatch(clearTransactionEvent()), 1000);
                            return false;
                        }
          ...
          <div className="send-notice">
            <p>
              Sending Switcheo.Network (SWH) NEP5 tokens require a balance of 0.00000001 GAS+. Only send SWH to a valid address that supports NEP5+ tokens on the NEO blockchain. When sending SWH to an exchange please ensure the address supports SWH tokens.
            </p>
            </div>
          </div>
        </div>
