...
const apiURL = val => {
  return "https://min-api.cryptocompare.com/data/price?fsym=GALA&tsyms=USD";
};
...
const validateForm = (dispatch, gala_balance) => {
  ...
  if (
    ...
    dispatch(sendEvent(false, "You cannot send fractional amounts of Gala."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
  } else if (parseInt(sendAmount.value) > gala_balance) {
    // check for value greater than account balance
    dispatch(sendEvent(false, "You do not have enough GALA to send."));
    setTimeout(() => dispatch(clearTransactionEvent()), 1000);
    return false;
...
// perform send transaction for Galaxy
const sendGalaTransaction = async (dispatch, net, selfAddress, wif) => {
  ...
  if (validateForm(dispatch, gala_balance) === true) {
      if (gala_balance <= sendAmount.value) {
          dispatch(sendEvent(false, "You are trying to send more GALA than you have available."));
          setTimeout(() => dispatch(clearTransactionEvent()), 2000);
          return true;
      } else {
          dispatch(sendEvent(true, "Sending GALAXY...\n"));
          try {
              ...
              console.log("sending gala =" + err.message);
        ...
        >
          <div>
            ...
            <div className="center modal-alert top-20">
              <strong>Confirm sending {sendAmount} GALA to {sendAddress}</strong>
            </div>
...
class SendGALA extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: true,
      ...
      <div>
        ...
        <div id="send">
          <div className="row dash-chart-panel">
            ...
            ).format("0,0.0000")} <span className="dbc-price"> GALA</span></span><br />
              <span className="market-price">{numeral(this.props.gala * this.props.marketGALAPrice).format("$0,0.00")} USD</span>
            </div>
                  ...
                  <button
                    ...
                    onClick={() => {
                        ...
                        if (parseFloat(sendAmount.value) <= 0) {
                            dispatch(sendEvent(false, "You cannot send negative amounts of an GALA."));
                            setTimeout(() => dispatch(clearTransactionEvent()), 1000);
                            return false;
                        }
...
SendGALA = connect(mapStateToProps)(SendGALA);

export default SendGALA;
