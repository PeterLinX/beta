...
const signAndsendTransaction = async (net, account) => {
	//Sign Transaction
	let fromAccount = account;
...
const StatusMessage = ({ sendAmount, sendAddress, handleCancel, handleConfirm, asset,action }) => {
    let message = (
		<Modal
			isOpen={true}
			closeTimeoutMS={5}
		...
		>
			<div>
				...
				<div className="center modal-alert top-20">
					<strong>Confirm {action} {sendAmount} {asset} to {sendAddress} by selecting Sign Tx on your Ledger Nano S.</strong>
				</div>
	...
	render() {
      ...
      { this.state.modalSignStatus?
		...
		<StatusMessage
				...
				handleConfirm ={() => {
				signAndsendTransaction(net,account)
				this.setState({
				    modalSendStatus: false
				})
				}}
		...
		<div id="send">
				    ...
				    if (parseFloat(sendAmount.value) <= 0) {
						...
						encodeTransactionAndcreateSignature(
							net,
							address,
							account,
							selectedAsset
