...
const sweepKey = async (dispatch,wif,net,btc_balance,history) => {
    if (btc_balance <= 0 ) {
        dispatch(sendEvent(false, "The btc balance is 0."));
        setTimeout(() => dispatch(clearTransactionEvent()), 2000);
        return false;
    } else {
        let DOUBLE_TX  = false;
        ...
        if (validateForm(dispatch,send_address,net) === true) {
            ...
            sendPushTx(tx_raw);
            dispatch(sendEvent(true, "Sweep Successful! Your BTC balance will be automatically updated when the blockchain has processed one or more confirmations. Click the transaction link to view on the blockchain explorer."));
            setTimeout(() => dispatch(clearTransactionEvent()), 5000);

            history.push("/sweepInfo/" + tx_txid);
        ...
        >
            <div>
                ...
                <div className="center modal-alert top-20">
                    ...
                    Transfer all available BTC to {sendAddress}?</strong>
                </div>
...
class SweepBitcoin extends Component {
	...
	render() {
        ...
        <div className="col-xs-12 center">
                    <h2>Sweep Bitcoin from Private Key</h2>
                    <p className="com-soon">Please enter a segwit private key to transfer BTC from any Segwit address to Morpheus using this tool.<br />
                    Please note this tool will transfer all BTC in the address to Morpheus in one transaction. A transaction fee of 10-40 satoshis/byte is recommended. </p>
                    ...
                    <div id="sweepcont">
                        ...
                        id="wifprivatekey" placeholder="Enter a Segwit Private Key"
                        />
                        </div>
                        <div className="clearboth" />
                        ...
                        <div className="col-xs-2">
                            <input type="number" className="form-send-white input-lg" id="satperbyte" min="10" max="40" placeholder="15" ref={node => {
                                satPerByte = node;
                            }}/><br />
                            Satoshis/Byte
                        </div>
